...
/**
 * 业务适配器工厂
 * <li>Spring 启动时自动扫描所有 ImportPlugin 实现类，按 bizType 注册;</li>
...
public class ImportPluginRegistry {
    ...
    public ImportPluginRegistry(List<ImportPlugin<?, ?>> plugins) {
        ...
        log.info("Excel import plugins registered: {} types, bizTypes: [{}]",
     ...
     * <p><b>工作机制：</b></p>
     * <ul>
     *     <li>适配器在 Spring 容器启动时由 {@link ImportPluginRegistry} 自动扫描注册</li>
     *     <li>所有实现了 {@link ImportPlugin} 接口的 Bean，按其 {@link ImportPlugin#getBizType()} 返回值建立映射</li>
     *     <li>若同一 bizType 被多个适配器实现，容器启动时会抛出 {@link IllegalStateException}，遵循快速失败原则</li>
     * </ul>
     *
     * <p><b>返回值说明：</b></p>
     * <ul>
     *     <li>返回类型为 {@code ImportPlugin<?, ?>}（通配符类型），表示适配器具体的泛型参数（DTO 和 Entity 类型）由适配器实现类自身决定</li>
     ...
     * <p><b>调用示例：</b></p>
     * <pre>{@code
     * ImportPlugin<?, ?> adapter = adapterFactory.getImportAdapter("USER_IMPORT");
     * // 调用方会自动推断泛型类型，无需显式转换
     ...
     */
    public ImportPlugin<?, ?> getImportPlugin(String bizType) {
        ImportPlugin<?, ?> adapter = pluginMap.get(bizType);
        if (adapter == null) {
