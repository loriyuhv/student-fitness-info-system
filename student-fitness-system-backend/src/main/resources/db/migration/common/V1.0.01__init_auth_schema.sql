...
(
    ...
    `source`      TINYINT      NOT NULL DEFAULT 0 COMMENT '用户来源：0-导入（IMPORT），1-同步（SYNC），2-手动录入（MANUAL）',
    `status`      TINYINT      NOT NULL DEFAULT 1 COMMENT '业务启用状态：0-禁用（无法登录/分配权限），1-启用（正常）',
    `deleted`     TINYINT      NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除，1-已删除（参与唯一索引，允许同名恢复）',
...
(
    ...
    `status`         TINYINT     NOT NULL DEFAULT 0 COMMENT '会话在线状态：1-在线（令牌有效），0-已下线（令牌已失效或被注销）',
    `deleted`        TINYINT     NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除，1-已删除（用于数据清理归档，默认保留）',
...
(
    ...
    `status`      TINYINT      NOT NULL DEFAULT 1 COMMENT '业务启用状态：0-禁用（该权限不可被授予角色，已有授权失效），1-启用（可正常分配）',
    `deleted`     TINYINT      NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除，1-已删除（参与唯一索引，允许删除后重新创建同名权限）',
    `create_time` DATETIME              DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间（自动生成）',
    `update_time` DATETIME              DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录最后更新时间（自动更新）',
...
(
    ...
    `status`      TINYINT NOT NULL DEFAULT 1 COMMENT '关联状态：0-禁用（该用户的该角色权限立即失效，但关联关系保留），1-启用（正常授予，权限生效）',
    `deleted`     TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除，1-已删除（参与唯一索引，允许删除后重新绑定相同角色）',
...
(
    ...
    `status`      TINYINT NOT NULL DEFAULT 1 COMMENT '关联状态：0-禁用（该角色的该权限立即失效，但关联关系保留），1-启用（正常授予，权限生效）',
    `deleted`     TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除，1-已删除（参与唯一索引，允许删除后重新绑定相同权限）',
    `create_time` DATETIME         DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间（自动生成，记录授权时间）',
    `update_time` DATETIME         DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录最后更新时间（自动更新，记录状态变更或重新授权时间）',
